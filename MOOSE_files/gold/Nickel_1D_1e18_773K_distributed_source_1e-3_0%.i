...
#         2- There are two type of sinks in system: uniformly distributed fixed sinks and grain boundaries
...
[Functions]
  [./dist_source]
    type = ParsedFunction
    value = 'k0:=9.037e-15;mu:=25;sigma:=5;if(x>mu,k0*exp(-((x-mu+sigma)^2)/(2*sigma^2))/(0.4*(2*acos(-1))^0.5),k0*exp(-((x-mu+sigma)^2)/(2*sigma^2))/(0.4*(2*acos(-1))^0.5)+0.25*k0*(1-pow((x/mu),2)))'
  [../]
[]
#@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
#--------------------------------------------------Kernels-----------------------------------------------
...
[Outputs]
  # exodus = true
  file_base = Nickel_1D_1e18_773K_distributed_source_1e-3_0%
  [./exodus]
    type = Exodus
